	...
	<xsl:template match="sujets-liees-list">
		<div style="margin-bottom: 0.5em;">
			...
			<xsl:for-each select="sujet-liee">
				...
				<a href='{$rebondUrl}/?simpleSearchCriterias=%3Csearch-criterias%3E%3Csujet%3E{value}%3C%2Fsujet%3E%3C%2Fsearch-criterias%3E'><xsl:value-of select="."/></a>
			</xsl:for-each>
		</div>
	</xsl:template>
			...
			<xsl:for-each select="ranked-authority-indice-cdu">
				...
				<div class="{$authorityClass}" style="display: {$authorityDisplay};">
					...
					<xsl:for-each select="indice-cdu">
						...
						<div>Dernier numéro reçu : <xsl:value-of select="dernier-numero"/></div>
